...
export abstract class BasePrismaStrategy {
    ...
    protected createBaseClient(databaseUrl: string): any {
        const adapter = this.createAdapter(databaseUrl);
        const ClientClass = this.config.PrismaClientClass;

        if (!ClientClass) {
            throw new Error('PrismaClientClass is required in config. Provide it via PrismaModule.forRoot().');
        }

        const prismaClient = new ClientClass({
            adapter,
    ...
    protected setupQueryLogging(prismaClient: any): void {
