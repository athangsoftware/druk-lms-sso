...
@Component({
  ...
  imports: [BaseInput, NgClass, CdkConnectedOverlay],
  templateUrl: './select-dropdown-field.html',
})
export class SelectDropdownField<T> {
  ...
  // Selected option derived from displayValue
  selectedOption = computed(() => {
    ...
    return this.options().find(item => {
      ...
      if (this.valueProperty()) {
        ...
        return this.getIdentifier(item) === this.getIdentifier(value as T);
      }
  ...
  protected onItemSelect(item: T): void {
    ...
    if (isSelected) {
      ...
      newValue = this.valueProperty() ? this.getValue(item) : item;
    }
  ...
  protected isOptionSelected(item: T): boolean {
    const controlValue = this.displayValue();
    ...
    if (this.valueProperty()) {
      ...
      return this.getIdentifier(controlValue as T) === itemId;
    }
