import { Component, input, OnInit, output, signal, model, effect, inject, ChangeDetectorRef, untracked } from "@angular/core";
...
@Component({
  selector: "ui-date-range-selection",
  imports: [NgClass],
  standalone: true,
  templateUrl: "./date-range-selection.html",
})
export class DateRangeSelection implements OnInit {
  // Inputs
  value = model<DateRangeEvent | null>(null);
  ...
  constructor() {
    // Sync internal state when model value changes
    // Sync internal state when model value changes
    effect(() => {
      const val = this.value();
      untracked(() => {
        if (val?.startDate && val?.endDate) {
          this.startDate.set(new Date(val.startDate));
          this.endDate.set(new Date(val.endDate));
          // Only update active month/year if not already set or if explicitly needed
          // For now, let's keep it simple and update view to start date
          this.activeMonth.set(this.startDate()!.getMonth());
          this.activeYear.set(this.startDate()!.getFullYear());
        } else {
          // If value is cleared externally, reset internal state
          this.startDate.set(null);
          this.endDate.set(null);
          // Don't reset active view to today, keep user context
        }
  ...
  onDaySelected(day: number) {
    ...
    console.log('onDaySelected', day, selectedDate, 'selectingStart:', this.selectingStart());
